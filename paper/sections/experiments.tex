...
reproducible MAST-FC2 trigger we identify. As a control, we re-ran
both \textsc{a2a} $\times$ \textsc{centralized} and \textsc{native}
$\times$ \textsc{centralized} at $T = 0$ with the \texttt{gpt-4o}
worker; both stayed at FC2 = 0 (5/5 valid trials each), confirming
that the non-zero FC2 in \textsc{mcp} $\times$ \textsc{centralized} is
specific to the protocol, not a property of the centralized topology
or of the \texttt{gpt-4o} worker.
