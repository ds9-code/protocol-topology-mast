...
\textsc{mcp} $\times$ \textsc{centralized} retains FC2 = 0.23 even at
$T = 0.0$ (13 valid trials replicated across \texttt{gpt-4.1-mini} and
\texttt{gpt-4o} workers), driven by orchestrators wrapping their own
integration step in \texttt{<mcp:message>} envelopes and executors
mis-parsing the partial envelopes. FC1 (specification) and FC3 (verification) show no
...
by replicating to 10--15 trials per cell, but the headline ANOVA
$p$-value (0.025) is still at the boundary of significance.
...
inherits annotator bias; in spot checks with \texttt{gpt-4o-mini} as
the annotator we found a few traces relabelled (typically dropping a
2.x code that gpt-3.5-turbo had flagged), so absolute rates are
annotator-sensitive even though within-sweep comparisons remain
