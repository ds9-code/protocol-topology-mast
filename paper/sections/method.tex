...
Each agent is a single OpenAI \textsc{gpt-4o-mini} chat completion seeded with a role
system message identifying its name (\textsc{orchestrator} or \textsc{executor}\_$i$),
the peer set, and the protocol-specific scaffolding. The three protocols differ only
...
\texttt{agentdash.annotator} (model: \textsc{gpt-4o-mini}), which returns the MAST
failure-mode dictionary\,\cite{mast2024}. We aggregate the 14 failure-mode codes into
...
\textsc{gpt-4o-mini}, \texttt{temperature}=0.7, \texttt{max\_tokens}=400 per agent
turn, \texttt{timeout}=60s per OpenAI call, $N_{\text{trials}}=5$. Sweep~1 holds
